...
class BigNum(object):
  ...
  def slow_mul(self, other):
    ...
    '''
    # fast_result = self.fast_mul(other)
    # return fast_result

    # result = BigNum.zero(1 + len(self.d) + len(other.d))
    # for i in range(len(self.d)):
    #     carry = Byte.zero()
    #     for j in range(len(other.d)):
    #         product = (self.d[i] * other.d[j])
    #         digit = product + result.d[i + j].word() + carry.word()
    #         result.d[i + j] = digit.lsb()
    #         carry = digit.msb()
    #     result.d[i + j + 1] = carry
    # return result.normalize()

    self_value = 0
    other_value = 0
    for i in range(len(self.d)-1, -1, -1):
        self_value = self_value << 8
        self_value += int(self.d[i].hex(), 16)
    for i in range(len(other.d)-1, -1, -1):
        other_value = other_value << 8
        other_value += int(other.d[i].hex(), 16)
    product = self_value * other_value
    result = BigNum.zero(1 + len(self.d) + len(other.d))
    i = 0
    while product > 0:
        remainder = product % 256
        remainder_tohex = [(hex(remainder >> 4))[2], (hex(remainder & 0xF))[2]]
        result.d[i] = Byte.from_hex(remainder_tohex)
        i = i + 1
        product = product >> 8
    return BigNum(result.d[:], None, True).normalize()
  ...
  def slow_divmod(self, other):
    ...
    for i in range(len(other.d)-1, -1, -1):
        other_value = other_value << 8
        other_value += int(other.d[i].hex(), 16)
    Remainder = self_value % other_value
    Quotient = self_value // other_value
    result = BigNum.zero(1 + len(self.d) + len(other.d))
    quotient = BigNum.zero(1 + len(self.d) + len(other.d))
    i = 0
    product = Remainder
    while product > 0:
        remainder = product % 256
    ...
    while product > 0:
        remainder = product % 256
        remainder_tohex = [(hex(remainder >> 4))[2], (hex(remainder & 0xF))[2]]
        quotient.d[i] = Byte.from_hex(remainder_tohex)
        i = i + 1
        product = product >> 8
    return (BigNum(quotient.d[:], None, True).normalize(),
            BigNum(result.d[:], None, True).normalize())
