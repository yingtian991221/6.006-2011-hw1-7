...
class PriorityQueue():
    ...
    def min_heapify(self):
        ...
        while i <= length // 2:
            left = i * 2
            right = i * 2 + 1
            if left <= length and self.heap[i - 1] > self.heap[left - 1]:
                small = left
            else:
                small = i
            if right <= length and self.heap[small - 1] > self.heap[right - 1]:
                small = right
            if small == i:
                return
            else:
                temp = self.heap[small - 1]
                self.heap[small - 1] = self.heap[i - 1]
                self.heap[i - 1] = temp
                i = small
    ...
    def append(self, num):
        ...
        if i == 0:
            print("already null")
            return
        while i // 2 >= 1 and self.heap[i - 1] < self.heap[i // 2 - 1]:
            temp = self.heap[i - 1]
